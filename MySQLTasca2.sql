...
SELECT p.nombre AS nombre_producto, p.precio, f.nombre AS nombre_fabricante FROM producto p JOIN fabricante f ON f.codigo = p.codigo_fabricante ORDER BY f.nombre ASC;
...
SELECT p.nombre AS nombre_producto, p.precio, f.nombre AS nombre_fabricante FROM producto p JOIN fabricante f ON f.codigo = p.codigo_fabricante ORDER BY p.precio ASC LIMIT 1;
...
SELECT p.nombre AS nombre_producto, p.precio, f.nombre AS nombre_fabricante FROM producto p JOIN fabricante f ON f.codigo = p.codigo_fabricante WHERE p.precio >= 180 ORDER BY p.precio DESC, p.nombre ASC;
...
SELECT DISTINCT apellido1, apellido2, nombre FROM persona WHERE tipo = "alumno" ORDER BY apellido1 ASC, apellido2 ASC, nombre ASC;
SELECT DISTINCT apellido1, apellido2, nombre FROM persona WHERE tipo = "alumno" AND telefono IS NULL;
SELECT DISTINCT * FROM persona  WHERE  tipo = 'alumno' AND YEAR(fecha_nacimiento) = 1999;
SELECT DISTINCT p.* FROM persona p JOIN profesor f ON p.id = f.id_profesor WHERE p.telefono IS NULL AND p.nif LIKE '%k';
SELECT * FROM asignatura WHERE cuatrimestre = 1 AND curso = 3 AND id_grado = 7;
SELECT p.apellido1, p.apellido2, p.nombre, d.nombre AS nombre_departamento FROM persona p JOIN profesor f ON p.id = f.id_profesor JOIN departamento d ON f.id_departamento = d.id  ORDER BY p.apellido1 ASC, p.apellido2 ASC, p.nombre ASC;
...
SELECT d.nombre AS departamento, p.apellido1 AS primer_apellido, apellido2 AS segundo_apellido, p.nombre AS nombre FROM persona p JOIN profesor f ON p.id = f.id_profesor LEFT JOIN departamento d ON f.id_departamento = d.id ORDER BY d.nombre ASC, p.apellido1 ASC, p.apellido2 ASC, p.nombre ASC;
